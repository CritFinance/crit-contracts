...
module.exports = ({web3} = {}) => {
    ...
    const fastForwardTo = async time => {
        ...
        if (time < now) throw new Error(`Time parameter (${time}) is less than now ${now}. You can only fast forward to times in the future.`)
    ...
    const assertEventEqual = (actualEventOrTransaction, expectedEvent, expectedArgs) => {
        ...
        const event = Array.isArray(actualEventOrTransaction.logs) ? actualEventOrTransaction.logs[0] : actualEventOrTransaction
    ...
    const assertEventsEqual = (transaction, ...expectedEventsAndArgs) => {
        if (expectedEventsAndArgs.length % 2 > 0) throw new Error('Please call assert.eventsEqual with names and args as pairs.')
        if (expectedEventsAndArgs.length <= 2) throw new Error('Expected events and args can be called with just assert.eventEqual as there\'s only one event.')
    ...
    const assertBNClose = (actualBN, expectedBN, varianceParam = '10') => {
        ...
        assert.ok(actual.gte(expected.sub(variance)), `Number is too small to be close (Delta between actual and expected is ${actualDelta.toString()}, but variance was only ${variance.toString()}`)
        assert.ok(actual.lte(expected.add(variance)), `Number is too large to be close (Delta between actual and expected is ${actualDelta.toString()}, but variance was only ${variance.toString()})`)
    }
    ...
    const loadLocalUsers = () => {
        return accounts.map(({privateKey}) => ({
            private: privateKey, public: web3.eth.accounts.privateKeyToAccount(privateKey).address
        }))
    }
    ...
    const onlyGivenAddressCanInvoke = async ({fnc, args, accounts, address = undefined, skipPassCheck = false, reason = undefined}) => {
        for (const user of accounts) {
            if (user === address) {
                continue
            }

            await assertRevert(fnc(...args, {from: user}), reason)
        }
    ...
    const assertions = Object.assign({}, assert, {
    ...
    return {
        mineBlock, fastForward, fastForwardTo, takeSnapshot, restoreSnapshot, currentTime, multiplyDecimal, divideDecimal, multiplyDecimalRound, divideDecimalRound, powerToDecimal,

        toUnit, fromUnit, toPreciseUnit, fromPreciseUnit,

        assertEventEqual, assertEventsEqual, assertBNEqual, assertBNNotEqual, assertBNClose, assertBNGreaterThan, assertBNGreaterEqualThan, assertBNLessThan, assertBNLessEqualThan, assertDeepEqual, assertInvalidOpcode, assertUnitEqual, assertUnitNotEqual, assertRevert,

        getEthBalance, bytesToString, loadLocalUsers, isCompileRequired,
